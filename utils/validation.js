...
const carDataSchema = Joi.object({
  ...
  vin: Joi.string().length(9).required().messages({
    ...
    'string.length': 'Reģistrācijas dokumenta numuram (vin) jābūt tieši 9 simboliem.',
  }),
});
...
const checkoutSchema = Joi.object({
  ...
  selectedOfferId: Joi.string()
    .valid(...process.env.NEXT_PUBLIC_INSURANCE_COMPANIES.split(','))
    .required()
    .messages({
...
export const validateCarRegistration = (data) =>
  carDataSchema.validate(data, { abortEarly: false });

export const validateCheckout = (data) => checkoutSchema.validate(data, { abortEarly: false });
